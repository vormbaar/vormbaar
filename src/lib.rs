...
impl From<ExprTree> for Option<Expr> {
    fn from(value: ExprTree) -> Self {
        match value {
            ExprTree::Value(a) => Some(Expr::Value(a.into())),
            ExprTree::Const(a) => Some(Expr::Const(a)),
            ExprTree::Var(a) => Some(Expr::Var(a)),
            ExprTree::Arg(a) => Some(Expr::Arg(a)),
            ExprTree::Item => Some(Expr::Item),
            _ => None,
        }
    }
}
...
impl From<&ExprTree> for Option<Expr> {
    fn from(value: &ExprTree) -> Self {
        match value {
            ExprTree::Value(a) => Some(Expr::Value(a.clone().into())),
            ExprTree::Const(a) => Some(Expr::Const(a.clone())),
            ExprTree::Var(a) => Some(Expr::Var(a.clone())),
            ExprTree::Arg(a) => Some(Expr::Arg(a.clone())),
            ExprTree::Item => Some(Expr::Item),
            _ => None,
        }
    }
...
pub enum Expr {
    ...
    Drop(Ref),
    Return(Ref),
    Call(String, BTreeMap<String, Box<Expr>>),
    Ref(Ref),
}
...
impl Expr {
    ...
    pub fn get_arguments(&self, fun: &Function) -> Vec<String> {
        ...
        match self {
            Expr::Value(_) => (),
            Expr::Ref(r) => args.extend(fun[**r].get_arguments(fun).into_iter()),
            Expr::Const(_) => (),
            Expr::Var(_) => (),
    ...
    pub fn flatten(self, container: &mut Vec<Expr>) {
            ...
            match expr {
                ...
                ExprTree::Call(name, args) => {
                    let e = Expr::Call(
                        ...
                        args.iter()
                            .map(|(name, value)| {
                                Option::<Expr>::from(value.clone())
                                    .map(|v| (name.to_owned(), Box::new(v)))
                            })
                            .flatten()
                            .collect(),
                    );
        ...
        for item in container.iter_mut() {
            match item {
                ...
                Expr::BinaryOp(_, left, right) => {
                    *left = Ref(max_index - **left);
                    *right = Ref(max_index - **right);
                }
                Expr::UnaryOp(_, expr) => {
                    *expr = Ref(max_index - **expr);
                }
                Expr::If(comp, if_true, if_false) => {
                    *comp = Ref(max_index - **comp);
                    *if_true = if_true.iter().map(|v| Ref(max_index - **v)).collect();
                    *if_false = if_false.iter().map(|v| Ref(max_index - **v)).collect();
                }
                Expr::ConstAssign(_, expr) => {
                    *expr = Ref(max_index - **expr);
                }
                Expr::For(range, body) => {
                    *range = match range {
                        Range::Values(values) => Range::Values(values.clone()),
                        Range::Range {
                            start,
                            stop,
                            step,
                            ...
                            start: Ref(max_index - **start),
                            stop: Ref(max_index - **stop),
                            step: Ref(max_index - **step),
                            mode: mode.clone(),
                        },
                    };
                    *body = body.iter().map(|v| Ref(max_index - **v)).collect();
                }
                Expr::VarAssign(_, expr) => {
                    *expr = Ref(max_index - **expr);
                }
                    ...
                    *args = args
                        ...
                        .map(|(name, arg)| {
                            (
                                name.clone(),
                                match *arg.clone() {
                                    Expr::Ref(Ref(arg)) => Expr::Ref(Ref(max_index - arg)).into(),
                                    other => other.into(),
                                },
                            )
                        })
        ...
        match self {
            ...
            ExprTree::Call(name, args) => {
                let e = Expr::Call(
                    ...
                    args.iter()
                        .map(|(name, e)| {
                            Option::<Expr>::from(e).map(|v| (name.to_owned(), v.into()))
                        })
                        .flatten()
                        .collect(),
                );
...
impl VM {
    ...
    pub fn eval_expression(
        ...
        match expr {
            Expr::Ref(Ref(index)) => self.eval_expression(
                fun,
                local_context,
                args,
                iteration_item,
            ...
            Expr::Call(name, call_args) => {
                ...
                self.call_function(
                    ...
                    Some(
                        &args
                            .iter()
                            .map(|(name, arg)| (name.to_owned().clone(), arg.clone()))
                            .collect(),
                    ),
                )
...
G        calling_context: &FunctionContext,
        ...
        Ok(self.eval_expressions(fun, calling_context, iteration_item, &body, args, 0)?)
    }
